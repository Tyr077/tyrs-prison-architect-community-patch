<#
  ...
  hard-coded allow list, not a staff type and not under an escape/override flag. The list predates the
  later DLC entities: RehabilitatedPrisoner (0x1C2), AnimalTherapist (0x1BF), FireSafetyTeacher (0x1FD)
  ...
  requests a guard. Result: they stand at the door forever. Double visitor doors happen to take a branch
  in TryPassThroughDoor that requires a different bit, so a guard is requested and they get through.
...
AddEdit $SITE ($new.ToArray()) 'Door::Open visitor-door class: refuse only prisoners instead of an allow list that misses the later DLC entities'
...
$doc = [ordered]@{
    ...
    description = 'Reformed prisoners, animal therapists, fire safety teachers, delivery men and other non-prisoner NPCs added in later DLC could not open visitor doors or visitor gates and never asked a guard to open them, so they stood at the door forever. Every non-prisoner can now open visitor doors, which is the rule the game already uses for path-finding. Prisoners are still refused.'
